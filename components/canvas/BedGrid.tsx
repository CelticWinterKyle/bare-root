...
import { Sprout, X as CloseIcon, CheckSquare, Move, Sun, Leaf, MousePointer2, Eye } from "lucide-react";
...
type Props = {
  ...
  /** Scrubbed to a future month: new plantings anchor here (window +
   *  reminders), and the grid is rendering that month's occupancy. */
...
export function BedGrid({ bedId, gardenId, gridCols, gridRows, cellSizeIn, cells, seasonId, userId, recentPlants, suggestionsLabel, plannedFor = null, isPro, prefillPlant, frost, canEdit = true, seedInventory = [], familyHistory = [] }: Props) {
  ...
  const footprintBounds = (() => {
    const m = new Map<
      string,
      { minR: number; maxR: number; minC: number; maxC: number; name: string | null; variety: string | null; category: string; temporal: string; occupiesFrom: Date | null }
    >();
    displayCells.forEach((cell, i) => {
      ...
      const b = m.get(pid) ?? { minR: r, maxR: r, minC: c, maxC: c, name: null, variety: null, category: "OTHER", temporal: "current", occupiesFrom: null };
      ...
      if (cell.planting) {
        ...
        b.temporal = cell.planting.temporal ?? "current";
        b.occupiesFrom = cell.planting.occupiesFrom ?? null;
      }
      m.set(pid, b);
    });
    return [...m.entries()];
  ...
  function handleCellClick(cell: CellData) {
    ...
    if (pendingPlant) {
      ...
      startPrefillAssign(async () => {
        try {
          const result = await assignPlant(cell.id, plant.id, seasonId, undefined, plannedFor ? { plannedFor } : undefined);
  ...
  function onDragEnd(e: DragEndEvent) {
    ...
    if (source.kind === "plant") {
      ...
      startPrefillAssign(async () => {
        try {
          const result = await assignPlant(targetCell.id, plant.id, seasonId, undefined, plannedFor ? { plannedFor } : undefined);
                    ...
                    {!sunMode && (
                      <div className="absolute inset-0 pointer-events-none">
                        {footprintBounds.map(([pid, b]) => {
                          ...
                          // Temporal ghosting (scrubber): future plantings are
                          // translucent + dashed with a month pill; past ones
                          // (scrubbed history) are desaturated ghosts. Current
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                border: isSel
                                  ? "2px solid #14260a"
                                  : isFuture
                                  ? "2px dashed rgba(253,253,248,0.7)"
                            ...
                            >
                              ...
                              {isFuture && b.occupiesFrom && (
                                ...
                                >
                                  {new Date(b.occupiesFrom).toLocaleDateString("en-US", { month: "short" })}
                                </span>
                              )}
                            </div>
                          );
