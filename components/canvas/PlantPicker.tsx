  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {!query && recentPlants.length > 0 && (
        <p className="text-xs text-[#9E9890] mb-2">Recently used</p>
      )}

      <div className="flex-1 overflow-y-auto space-y-1">
        {results.length === 0 ? (
          <div className="text-center py-8 text-[#9E9890] text-sm">
          ...
          results.map((plant) => (
            <button
              ...
              className="w-full flex items-center gap-3 p-2.5 rounded-lg hover:bg-[#F5F0E8] transition-colors text-left disabled:opacity-50"
            >
              {plant.imageUrl ? (
                <div className="w-10 h-10 rounded-lg overflow-hidden bg-[#F5F0E8] shrink-0 relative">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm text-[#1C1C1A] truncate">{plant.name}</p>
              ...
              {isAssigning && assigningId === plant.id && (
                <Loader2 className="w-4 h-4 animate-spin text-[#6B8F47] shrink-0" />
              )}
            </button>
          ))
        )}
