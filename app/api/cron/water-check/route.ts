...
export async function GET(req: Request) {
  ...
  const result = await withCronRun("water-check", async () => {
    ...
    if (caches.length === 0) return { ok: true, remindersCreated: 0, sent: 0 };
    ...
    return { ok: true, remindersCreated: created.count, sent: created.count };
