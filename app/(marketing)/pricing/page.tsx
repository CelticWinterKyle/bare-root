...
function Cell({ value }: { value: boolean | string }) {
  if (typeof value === "string") return <span className="text-sm text-[#1C1C1A]">{value}</span>;
  if (value) return <Check className="w-4 h-4 text-[#4A7C2F] mx-auto" />;
  return <span className="text-[#E8E2D9] text-lg leading-none">—</span>;
}
  ...
  return (
    <main className="min-h-screen bg-[#FAF7F2]">
      <nav className="flex items-center justify-between px-8 py-6 max-w-5xl mx-auto">
        <Link href="/" className="font-display text-2xl font-semibold text-[#2D5016]">Bare Root</Link>
        <div className="flex items-center gap-4">
          <Link href="/sign-in" className="text-sm text-[#6B6560] hover:text-[#1C1C1A] transition-colors">Sign in</Link>
          <Link href="/sign-up" className="text-sm bg-[#2D5016] text-white px-4 py-2 rounded-lg hover:bg-[#4A7C2F] transition-colors">Start free</Link>
        </div>
      </nav>

      <section className="max-w-5xl mx-auto px-6 pt-16 pb-8 text-center">
        <h1 className="font-display text-4xl font-semibold text-[#1C1C1A] mb-3">Simple, honest pricing</h1>
        <p className="text-[#6B6560] text-lg">Free to start. Upgrade when you want more.</p>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 pb-16">
        <div className="grid gap-4 sm:grid-cols-2 mb-16">
          ...
          <div className="bg-white border border-[#E8E2D9] rounded-2xl p-6">
            <p className="font-semibold text-lg text-[#1C1C1A] mb-1">Free</p>
            <p className="text-3xl font-bold text-[#1C1C1A] mb-1">$0</p>
            <p className="text-sm text-[#9E9890] mb-6">Forever</p>
            <Link
              href="/sign-up"
              className="block w-full text-center border border-[#2D5016] text-[#2D5016] font-medium py-3 rounded-xl hover:bg-[#F5F0E8] transition-colors text-sm mb-6"
            >
              Get started free
            </Link>
          </div>
          ...
          {/* Pro */}
          <div className="bg-[#2D5016] rounded-2xl p-6 text-white relative overflow-hidden">
            <div className="absolute top-4 right-4 text-[11px] bg-[#C4790A] px-2 py-0.5 rounded-full font-medium">7-day trial</div>
            ...
            <Link
              href="/sign-up"
              className="block w-full text-center bg-white text-[#2D5016] font-semibold py-3 rounded-xl hover:bg-white/90 transition-colors text-sm mb-1"
            >
              Start free trial
            </Link>
        ...
        {/* Comparison table */}
        <div className="bg-white rounded-2xl border border-[#E8E2D9] overflow-hidden mb-16">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#E8E2D9]">
                <th className="text-left px-5 py-4 text-sm text-[#9E9890] font-medium">Feature</th>
                <th className="text-center px-5 py-4 text-sm text-[#9E9890] font-medium">Free</th>
                <th className="text-center px-5 py-4 text-sm text-[#2D5016] font-semibold">Pro</th>
              </tr>
            </thead>
            <tbody>
              {COMPARISON.map((row, i) => (
                <tr key={row.feature} className={i % 2 === 0 ? "bg-white" : "bg-[#FAF7F2]"}>
                  <td className="px-5 py-3 text-sm text-[#1C1C1A]">{row.feature}</td>
        ...
        {/* FAQ */}
        <h2 className="font-display text-2xl font-semibold text-[#1C1C1A] mb-6">Questions</h2>
        <div className="space-y-4 mb-16">
          {FAQ.map((item) => (
            <div key={item.q} className="bg-white rounded-xl border border-[#E8E2D9] p-5">
              <p className="font-medium text-[#1C1C1A] mb-1.5 text-sm">{item.q}</p>
              <p className="text-[#6B6560] text-sm">{item.a}</p>
            </div>
          ))}
        </div>

        <div className="text-center">
          <p className="font-display text-2xl font-semibold text-[#1C1C1A] mb-4">Your best garden starts here.</p>
          <Link
            href="/sign-up"
      ...
      <footer className="border-t border-[#E8E2D9] py-8 text-center text-sm text-[#9E9890]">
        <div className="flex items-center justify-center gap-6 mb-3">
          <Link href="/" className="hover:text-[#1C1C1A]">Home</Link>
